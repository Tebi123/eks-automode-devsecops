# security.tf - Additional security configurations for EKS Auto Mode
...
# CloudWatch Log Group for EKS Cluster logs with encryption
resource "aws_cloudwatch_log_group" "eks_cluster" {
...
# Security Group rule to restrict API server access to specific IPs
resource "aws_security_group" "additional_cluster_sg" {
  name        = "${local.name}-additional-cluster-sg"
  description = "Additional security group for EKS cluster"
  vpc_id      = module.vpc.vpc_id

  # Allow HTTPS from specific CIDR blocks only
  ingress {
...
resource "aws_network_acl" "private_subnet_nacl" {
  ...
  # Allow outbound HTTPS for pulling images
  egress {
    protocol   = "tcp"
    rule_no    = 200
    action     = "allow"
  ...
  # Allow outbound HTTP for pulling images
  egress {
    protocol   = "tcp"
    rule_no    = 300
    action     = "allow"
  ...
  tags = merge(local.common_tags, {
    Name = "${local.name}-private-subnet-nacl"
  })
}

# GuardDuty for threat detection (if not already enabled in the account)
...
resource "aws_s3_bucket" "config" {
  bucket        = "${local.name}-aws-config-${random_id.config_bucket_suffix.hex}"
  force_destroy = true
...
resource "aws_iam_role" "config" {
  name = "${local.name}-aws-config-role"
